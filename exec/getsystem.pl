...
sub system_name {
    if (defined $options{'t'}) {
        given ($_[0]) {
            when(['t','g','e']) { return 'IBM'; }
            when(['l','s','h']) { return 'Cray'; }
            when(['m','v','p']) { return 'Dell'; }
        }
    } else {
        given ($_[0]) {
            ...
            when('h') { return 'Hank'; }
            when('m') { return defined $options{'o'} ? 'Venus' : 'Mars'; }
            when('v') { return defined $options{'o'} ? 'Mars' : 'Venus'; }
            when('p') { return 'Pluto'; }
        }
    }
}
...
sub get_dell_phase {
    return ( defined $options{'p'} ? '-p3' : '' );
}

my $hostname=`hostname`;
if ( $hostname =~ /^(t|g|e)(\d{1,2})\w\d/ ) {  # IBM iDataPlex nodes
    print system_name($1);
...
} elsif ( $hostname =~ /^(m|v|p)(\d{1,2})\w\d/ ) {  # Dell PowerEdge nodes
    print system_name($1);
    print '-p3' if defined $options{'p'};
} else {
    my $external_ip;
    ...
    given ($external_ip) {
        ...
        when('192.58.151') { print system_name('m') . get_dell_phase(); }
        when('192.58.152') { print system_name('v') . get_dell_phase(); }
        default { die "Unable to identify the system. You may not be running on a service node." }
    }
}
