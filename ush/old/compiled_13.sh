...
# Purpose:
#
# IT checklist, Compiled Code #13: "Have poor programming practices (e.g. GO TO, no
# comments, etc.) been removed from source?"
#
...
for f in $(find ./sorc/* -type f | xargs file | grep FORTRAN | awk -F ':' '{print $1}'); do
 echo "# of GOTO statements in $f: $(nocomment_dummy.sh $f | grep -ciE '(\b|^)GO\ ?TO\b')"
done
echo '#####################################'
# Comments:
for f in $(find ./sorc/* -type f); do
 wholecount=$(grep -Ev "^[[:space:]]*$" $f | wc -l)
 noncommcount=$(nocomment_dummy.sh $f | grep -Ev "^[[:space:]]*$" | wc -l)
 ...
 done < <(nocomment_dummy.sh $f -h | grep -Ev "^[[:space:]]*$")
 if [ $currentnc -gt $longestnc ]; then longestnc=$currentnc; fi
